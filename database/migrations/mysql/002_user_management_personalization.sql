-- Migration: User Management & Personalization (MySQL/MariaDB)
-- Version: 0.2.0
-- Created: 2025-11-11
...
CREATE INDEX idx_user_destinations_channel_type ON user_destinations(channel_type);
CREATE INDEX idx_user_destinations_destination ON user_destinations(destination);
CREATE INDEX idx_user_destinations_primary ON user_destinations(user_id, channel_type, is_primary);
...
CREATE INDEX idx_group_keywords_group_id ON group_keywords(group_id);
CREATE INDEX idx_group_keywords_keyword ON group_keywords(keyword);
...
CREATE INDEX idx_llm_prompts_channel_type ON llm_prompts(channel_type);
CREATE INDEX idx_llm_prompts_group_id ON llm_prompts(group_id);
CREATE INDEX idx_llm_prompts_language ON llm_prompts(language);
CREATE INDEX idx_llm_prompts_keyword ON llm_prompts(keyword);
CREATE INDEX idx_llm_prompts_priority ON llm_prompts(priority DESC);
CREATE INDEX idx_llm_prompts_enabled ON llm_prompts(enabled);
...
CREATE INDEX idx_ldap_syncs_source_type ON ldap_syncs(source_type);
CREATE INDEX idx_ldap_syncs_enabled ON ldap_syncs(enabled);
CREATE INDEX idx_ldap_syncs_last_sync_at ON ldap_syncs(last_sync_at);

-- Insert default groups
INSERT INTO groups (name, description, enabled)
SELECT 'Admin Users', 'Administrative users', 1
WHERE NOT EXISTS (SELECT 1 FROM groups WHERE name = 'Admin Users');

INSERT INTO groups (name, description, enabled)
SELECT 'System Users', 'Service accounts and system users', 1
...
INSERT INTO llm_prompts (name, channel_type, prompt_text, priority, enabled)
SELECT 'SMS Default', 'sms', 'Format the following content as a concise SMS message. Maximum 140 characters. Be direct and clear.', 0, 1
WHERE NOT EXISTS (SELECT 1 FROM llm_prompts WHERE name = 'SMS Default' AND channel_type = 'sms');

INSERT INTO llm_prompts (name, channel_type, prompt_text, priority, enabled)
SELECT 'WhatsApp Default', 'whatsapp', 'Format the following content as a WhatsApp message. Use emojis sparingly. Be conversational but professional.', 0, 1
WHERE NOT EXISTS (SELECT 1 FROM llm_prompts WHERE name = 'WhatsApp Default' AND channel_type = 'whatsapp');

INSERT INTO llm_prompts (name, channel_type, prompt_text, priority, enabled)
SELECT 'Slack Default', 'slack', 'Format the following content as a Slack message. Use Slack markdown formatting. Be concise and actionable.', 0, 1
WHERE NOT EXISTS (SELECT 1 FROM llm_prompts WHERE name = 'Slack Default' AND channel_type = 'slack');

INSERT INTO llm_prompts (name, channel_type, prompt_text, priority, enabled)
SELECT 'Teams Default', 'teams', 'Format the following content as a Microsoft Teams message. Use Teams markdown. Be professional and clear.', 0, 1
