...
License: Apache 2.0
Ownership: Cloud Dog
Description: Mock Email (SMTP) Adapter for testing - provides mock implementation of email adapter for unit and integration tests

Related Requirements: FR1.6
Related Tasks: T6, T17
...
class MockEmailAdapter(BaseChannelAdapter):
    ...
    def validate_destination(self, destination: str) -> bool:
        ...
        return bool(self.EMAIL_PATTERN.match(destination))
    
    async def send(self, delivery: Dict[str, Any]) -> SendResult:
        """Mock send - always succeeds instantly"""
        ...
        # Simulate sending
        tracking_id = f"mock-email-{uuid.uuid4().hex[:8]}"
        
        latency_ms = int((time.time() - start_time) * 1000)
        
        # Log to console (in production, this would actually send)
        logger.info(f"[MOCK EMAIL] To: {destination}")
        logger.info(f"[MOCK EMAIL] Tracking ID: {tracking_id}")
        logger.info(f"[MOCK EMAIL] Payload: {payload[:100]}...")
