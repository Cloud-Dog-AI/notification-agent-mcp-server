...
"""
AT1.17: Comprehensive Email Validation (Application tests)
...
def require_at117_env_loaded(test_config: Any) -> None:
    if not test_config.get("test.at117_env_loaded"):
        pytest.fail("❌ CRITICAL: AT1.17 env file not loaded! Use --env private/env-test-at117")
...
@pytest.fixture(scope="session")
def at117_max_wait(test_config: Any) -> float:
    v = test_config.get("test.at117.max_wait")
    if v is None or v == "":
        pytest.fail("❌ HARD FAIL: test.at117.max_wait not configured in env file")
    return float(v)


@pytest.fixture(scope="session")
def at117_poll_interval(test_config: Any) -> float:
    v = test_config.get("test.at117.poll_interval")
    if v is None or v == "":
        pytest.fail("❌ HARD FAIL: test.at117.poll_interval not configured in env file")
    return float(v)
...
def smtp_config(test_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if missing:
        pytest.fail(
            "❌ HARD FAIL: SMTP configuration incomplete for AT1.17. "
...
def smtp_channel_name(
    ...
    Ensures adapter registration and full CRUD coverage for AT1.17.
    """
    ...
    with httpx.Client(timeout=api_timeout) as client:
        ...
        channel_name = f"at117_smtp_{run_suffix}"
        payload = {
            "name": channel_name,
            "type": "smtp",
            "enabled": True,
