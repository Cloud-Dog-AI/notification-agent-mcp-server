...
"""
AT1.7 Translation Suite - shared fixtures

RULES.md compliance:
- API-only interactions (no direct DB access)
- No hardcoded configuration values (all required values must come from --env file)
- Fail hard if env not loaded
"""
...
def require_at17_env_loaded(test_config: Any) -> None:
    if test_config.get("test.at17_env_loaded") is False:
        pytest.fail("❌ CRITICAL: AT1.7 env marker explicitly false; set test.at17_env_loaded=true for this env")


@pytest.fixture(scope="session")
def api_base_url(test_config: Any, require_at17_env_loaded) -> str:
...
@pytest.fixture(scope="session")
def api_key(test_config: Any, require_at17_env_loaded) -> str:
    key = test_config.get("api_server.api_key")
    if not key:
        pytest.fail("❌ HARD FAIL: api_server.api_key not configured in env file")
    return str(key)


@pytest.fixture(scope="session")
def api_timeout(test_config: Any, require_at17_env_loaded) -> float:
...
def api_client(api_base_url: str, api_key: str, api_timeout: float):
    with httpx.Client(
        ...
        timeout=httpx.Timeout(timeout=api_timeout),
...
@pytest.fixture(scope="session")
def smtp_config(test_config: Any, require_at17_env_loaded) -> Dict[str, Any]:
    ...
    if missing:
        pytest.fail(
            "❌ HARD FAIL: SMTP configuration incomplete for AT1.7. "
...
@pytest.fixture(scope="session")
def smtp_channel_name(api_base_url: str, api_key: str, api_timeout: float, smtp_config: Dict[str, Any], request):
    """
    Create a dedicated, fully-configured SMTP channel via API.
    Ensures adapter registration and full CRUD coverage for AT1.7.
    """
    ...
    with httpx.Client(timeout=api_timeout) as client:
        ...
        channel_name = f"at17_smtp_{run_suffix}"
        payload = {
            "name": channel_name,
            "type": "smtp",
            "enabled": True,
