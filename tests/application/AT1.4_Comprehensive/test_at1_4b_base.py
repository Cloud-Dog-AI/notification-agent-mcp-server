...
def test_at1_4a_translation_summary(api_client, loopback_channel, test_output_dir, test_email_domain):
    """
    ...
    # Filter to 5000 char messages (summary needed)
    summary_cases = [tc for tc in test_cases if tc["size"] == 5000]
    
    for test_case in summary_cases[:3]:  # Test first 3 for now
        source_lang = test_case["source"]
        target_lang = test_case["target"]
        source_size = test_case["size"]
        target_summary_size = 400  # Target summary size
        ...
        # 3. Generate summary via API
        # Create message with target language preference and summary size limit
        message_payload = {
            "content": [{"type": "text", "body": source_content}],
            "destinations": [{
                "channel": channel_name,
                "address": f"test_{source_lang}_{target_lang}{test_email_domain}",
                "preferences": {
        ...
        while wait_time < max_wait:
            ...
            if state == 'sent':
                ...
                # Payload can be a list of content blocks or a dict
        ...
        assert target_lang_valid, \
            f"Summary not in {target_lang}: {target_indicators}"
        print(f"✅ Target language validated: {target_lang} ({len(target_indicators)} indicators)")
        
        # Validate target size (language-aware: wider tolerance for CJK and all languages)
        # Use 0.40 tolerance (40%) to accommodate LLM variance in summary generation
