...
"""Shared pytest fixtures for AT1.20 Media Support Test Suite"""
...
def restart_api_per_test(api_base_url, api_key):
    """
    AT1.20 exercises long-running media rendering/duplication paths, but the
    suite only needs a healthy API and cleared queued backlog between cases.
    """
    _, cancelled = _ensure_api_ready_for_test(
        api_base_url,
        api_key,
        timeout_seconds=60.0,
        context_label="AT1.20 test execution",
    )
    if cancelled:
        print(f"✅ AT1.20 cancelled {cancelled} stale queued message(s) before test")
